...
! function(window) {
    ...
    CitySelect.prototype.init = function() {
        ...
        _this.defaultSet = {
            provance: {
                n: options.provance.n,
                code: options.provance.code
            },
            city: {
                n: options.city.n,
                code: options.city.code
            },
            area: {
                n: options.area.n,
    ...
    CitySelect.prototype.change = function(data) {
        var _this = this;
        _this.defaultSet = {
            provance: {
                n: data.provance.n,
                code: data.provance.code
            },
            city: {
                n: data.city.n,
                code: data.city.code
    ...
    CitySelect.prototype.open = function() {
        ...
        if (defaultSet.provance.n != "") {
            _this.setNavTxt(0, defaultSet.provance.n, defaultSet.provance.code);
        } else {
            $cityElement.find('.cityselect-nav a').eq(0).addClass('crt').html('请选择');
        }

        if (defaultSet.city.n != "") {
            _this.loadCity();
            _this.setNavTxt(1, defaultSet.city.n, defaultSet.city.code)
        } else if (defaultSet.provance.n != "") {
            _this.loadCity();
        }

        if (defaultSet.area.n != "") {
            _this.loadArea();
            _this.ForwardView(false);
            _this.setNavTxt(2, defaultSet.area.n, defaultSet.area.code);
        } else if (defaultSet.city.n != "") {
            _this.loadArea();
            _this.ForwardView(false);
        }
    ...
    CitySelect.prototype.setNavTxt = function(index, txt, data) {
        ...
        $nav.eq(index).attr('data-param', data);
    ...
    CitySelect.prototype.bindItemEvent = function() {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function() {
            ...
            _this.setNavTxt(tag, $this.text(), $this.attr('data-param'));
            ...
            if (tag == 0) {
                ...
                defaultSet.provance.n = $nav.eq(0).html();
                defaultSet.provance.code = parseInt($nav.eq(0).attr('data-param'));
                defaultSet.city.n = $nav.eq(1).html();
                defaultSet.city.code = parseInt($nav.eq(1).attr('data-param'));
                defaultSet.area.n = $nav.eq(2).html();
                defaultSet.area.code = parseInt($nav.eq(2).attr('data-param'));
    ...
    CitySelect.prototype.loadProvance = function() {
        ...
        $.each(_this.citys, function(k, v) {
            arr.push($('<a class="' + (v.code == _this.defaultSet.provance.code ? 'crt' : '') + '" href="javascript:;" data-param="' + v.code + '"><span>' + v.n + '</span></a>').data({
                citys: v.c,
                tag: 0
            }));
        });
    ...
    CitySelect.prototype.loadCity = function() {
        ...
        $.each(cityData, function(k, v) {
            arr.push($('<a class="' + (v.code == _this.defaultSet.city.code ? 'crt' : '') + '" href="javascript:;" data-param="' + v.code + '"><span>' + v.n + '</span></a>').data({
                citys: v.a,
                tag: 1
            }));
        });
    ...
    CitySelect.prototype.loadArea = function() {
        ...
        $.each(areaData, function(k, v) {
            arr.push($('<a class="' + (v.code == _this.defaultSet.area.code ? 'crt' : '') + '" href="javascript:;" data-param="' + v.code + '"><span>' + v.n + '</span></a>').data({
                tag: 2
            }));
        });
