...
!function (window) {
    ...
    CitySelect.prototype.open = function () {
        ...
        if (defaultSet.provance) {
            _this.setNavTxt(0, defaultSet.provance.n);
        } else {
        ...
        if (defaultSet.city) {
            _this.loadCity();
            _this.setNavTxt(1, defaultSet.city.n)
        }

        if (defaultSet.area) {
            _this.loadArea();
            _this.ForwardView(false);
            _this.setNavTxt(2, defaultSet.area.n);
        }
    ...
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            if (tag == 0) {
                ...
                defaultSet.provance = eval('(' + $nav.eq(0).attr('data-param') + ')');
                defaultSet.city = eval('(' + $nav.eq(1).attr('data-param') + ')');
                defaultSet.area = eval('(' + $nav.eq(2).attr('data-param') + ')');
    ...
    CitySelect.prototype.returnValue = function () {
        var _this = this,
            defaultSet = _this.defaultSet;
            // console.log(JSON.stringify(defaultSet));
    ...
    CitySelect.prototype.loadProvance = function () {
        ...
        $.each(_this.citys, function (k, v) {
            arr.push($('<a class="' + (v.code == _this.defaultSet.provance.code ? 'crt' : '') + '" href="javascript:;" data-param="'+JSON.stringify(v)+'"><span>' + v.n + '</span></a>').data({
                citys: v.c,
                tag: 0
            }));
        });
    ...
    CitySelect.prototype.loadCity = function () {
        ...
        $.each(cityData, function (k, v) {
            arr.push($('<a class="' + (v.code == _this.defaultSet.city.code ? 'crt' : '') + '" href="javascript:;" data-param="'+JSON.stringify(v)+'"><span>' + v.n + '</span></a>').data({
                citys: v.a,
                tag: 1
            }));
        });
    ...
    CitySelect.prototype.loadArea = function () {
        ...
        $.each(areaData, function (k, v) {
            arr.push($('<a class="' + (v.code == _this.defaultSet.area.code ? 'crt' : '') + '" href="javascript:;" data-param="'+JSON.stringify(v)+'"><span>' + v.n + '</span></a>').data({tag: 2}));
        });
