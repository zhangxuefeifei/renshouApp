    ...
    auiDialog.prototype = {
        ...
        create: function(params,callback) {
            ...
            var footerHtml = '<div class="aui-dialog-footer">'+buttonsHtml+'</div>';
              // 自己新增的 2-18 <div class="aui-mask aui-mask-in1"> 并新增aui-mask-in1
            dialogHtml = '<div class="aui-mask aui-mask-in1"><div class="aui-dialog" id="aui-dialog">'+headerHtml+msgHtml+footerHtml+'</div></div>';
            document.body.insertAdjacentHTML('beforeend', dialogHtml);
            if(params.inputValue != ''){
              $('input').val(params.inputValue);
            }
            var auidg = document.getElementById('aui-dialog');
            auidg.addEventListener("touchstart", function(event) {
            ...
            if(dialogButtons && dialogButtons.length > 0){
                for(var ii = 0; ii < dialogButtons.length; ii++){
                    dialogButtons[ii].ontouchstart = function(){
                        if(callback){
                            if(params.input){
                                callback({
        ...
        open: function(){
            ...
            setTimeout(function(){
                ...
                document.querySelector(".aui-mask").classList.add("aui-mask-in1");
                document.querySelector(".aui-dialog").classList.add("aui-dialog-in");
            }, 10)
        ...
        close: function(){
            var self = this;
            document.querySelector(".aui-mask").classList.remove("aui-mask-in1");
            ...
            if (document.querySelector(".aui-dialog:not(.aui-dialog-out)")) {
                ...
                // 自己新增的 2-18 开始
               var All = document.querySelectorAll(".aui-mask.aui-mask-in1");
                for(var i=0;i<All.length;i++){
                  All[i].classList.remove("aui-mask-in1");
                }
                  // 自己新增的 2-18 结束
        ...
        remove: function(){
            ...
            if(document.querySelector(".aui-mask")){
              ...
              // 自己新增的 2-18 开始
              var AllMask = document.querySelectorAll(".aui-mask");
